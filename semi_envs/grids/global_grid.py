...
class OfficeEnvS(gym.Env):
    ...
    def step(self, action):
        ...
        if not (self.get_true_propositions()==""):
            self._hand.add(self.get_true_propositions())
    ...
    def reward_func(self):
        if self.mode == 'g':
            if self.agent == (4, 4): return 1
            else: return 0
        elif self.mode == 'eg':
            if self.agent == (4, 4) and 'e' in self._hand: return 1
            else: return 0
        elif self.mode == 'fg':
            if self.agent == (4, 4) and 'f' in self._hand: return 1
            else: return 0
        elif self.mode == 'efg':
            if self.agent == (4, 4) and 'e' in self._hand and 'f' in self._hand: return 1
            else: return 0
        else:
    ...
    def get_features(self):
        """
        Returns the features of the current state (i.e., the location of the agent and objects)
        """
        x,y = self.agent
        raise NotImplementedError
